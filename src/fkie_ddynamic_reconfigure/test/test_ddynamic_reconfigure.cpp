...
using namespace fkie_ddynamic_reconfigure;
...
TEST_F(DDynamicReconfigureTest, callbackTest)
{
  ...
  DDynamicReconfigure dd(nh);
  MockClass mock;
  // ddr.registerVariable< int >( "int_param", mock.int_param, boost::bind( fkie_ddynamic_reconfigure::paramCbInt, boost::ref( name ), #name, _1 ), desc, min, max );  

  dd.registerVariable<int>("int_param", mock.int_param_,boost::bind(&MockClass::intCallback, &mock, _1), "blabla int", 0, 100 );
  dd.registerVariable<double>("double_param", mock.double_param_, boost::bind(&MockClass::doubleCallback, &mock, _1), "blabla double", 0.0, 1.0 );
  dd.registerVariable<bool>("bool_param", mock.bool_param_, boost::bind(&MockClass::boolCallback, &mock, _1), "blabla bool" );
  dd.registerVariable<std::string>("str_param", mock.str_param_, boost::bind(&MockClass::strCallback, &mock, _1), "blabla string" );
  dd.PublishServicesTopics();
  ros::AsyncSpinner spinner(1);
  spinner.start();
...
int main(int argc, char** argv)
{
  ...
  ros::init(argc, argv, "fkie_ddynamic_reconfigure_test");
  ros::NodeHandle nh;
