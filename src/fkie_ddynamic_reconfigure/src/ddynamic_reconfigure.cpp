...
/**
 * Modified by Fraunhofer FKIE/CMS. Syntax changed.
 * Groups and initial parameter loading fixed
 */
...
void DDynamicReconfigure::getGroupStates( std::vector< dynamic_reconfigure::GroupState >* group_states, std::set<std::string>* groups_set ) const
{
  // this searches the given parameters for groups. The original group structure is the other way around, making this quite ugly

  std::set<std::string> groups_set_temp;
  if (groups_set == NULL)
    groups_set = &groups_set_temp;

  for (unsigned int i = 0; i < registered_int_.size(); ++i)
    groups_set->insert(registered_int_[i]->group_);
  for (unsigned int i = 0; i < registered_double_.size(); ++i)
    groups_set->insert(registered_double_[i]->group_);
  for (unsigned int i = 0; i < registered_bool_.size(); ++i)
    groups_set->insert(registered_bool_[i]->group_);
  for (unsigned int i = 0; i < registered_string_.size(); ++i)
    groups_set->insert(registered_string_[i]->group_);

  bool has_default = false;
  size_t c = 1;
  for (std::set<std::string>::iterator it = groups_set->begin(); it != groups_set->end(); it++)
  {
    dynamic_reconfigure::GroupState gs;
    gs.name = *it;

    if (gs.name.empty() || (gs.name == "Default"))
    {
      gs.name = "Default";
      gs.id = 0;
      has_default = true;
                      ...
                      const std::string &description, T min, T max, const std::string &group)
{
  ...
  getRegisteredVector<T>().push_back(
      boost::make_unique<PointerRegisteredParam<T>>(name, description, min, max, variable, std::map<std::string, T>(), "", group));
}
...
{
  ROS_DEBUG_STREAM("Called config callback of fkie_ddynamic_reconfigure");
...
dynamic_reconfigure::ConfigDescription DDynamicReconfigure::generateConfigDescription() const
{
  ...
  std::vector< dynamic_reconfigure::GroupState > group_states;

  getGroupStates( &group_states, &groups );

  config_description.dflt.groups = group_states;
  config_description.min.groups = group_states;
  config_description.max.groups = group_states;
  ...
  for (std::set<std::string>::iterator it = groups.begin(); it != groups.end(); it++)
  {
    dynamic_reconfigure::Group gp;
    gp.name = *it;
    gp.type = visual_type_;

    if (gp.name.empty() || (gp.name == "Default"))
    {
      gp.name = "Default";
      gp.id = 0;
      has_default = true;
...
template void DDynamicReconfigure::registerEnumVariable(
    ...
    std::map<std::string, int> enum_dict, const std::string &enum_description, const std::string &group);
...
template void DDynamicReconfigure::registerEnumVariable(
    ...
    std::map<std::string, double> enum_dict, const std::string &enum_description, const std::string &group);
...
template void DDynamicReconfigure::registerEnumVariable(
    ...
    std::map<std::string, bool> enum_dict, const std::string &enum_description, const std::string &group);
...
template void DDynamicReconfigure::registerEnumVariable(
    ...
    std::map<std::string, std::string> enum_dict, const std::string &enum_description, const std::string &group);
