...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn)
{
  ...
  *exploredVolumeCloud += *laserCloud;
  ...
  // printf("过滤后：%f %f %d\n", exploredVolume, exploredVolumeVoxelSize, exploredVolumeCloud->points.size());
  std_msgs::Float32 exploredVolumeMsg;
  exploredVolumeMsg.data = exploredVolume/totalspace;
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2>("/points2_out", 5, laserCloudHandler);
