...
bool doGripperControl(arm_controller::GripperControl::Request  &req, arm_controller::GripperControl::Response &res);
...
int main(int argc, char **argv){ 
  ...
  move_group_gripper = new moveit::planning_interface::MoveGroupInterface(planning_group_name2);
  ...
  ros::ServiceServer gripper_control_srv = nh.advertiseService("GripperControl", doGripperControl);
...
bool doGripperControl(arm_controller::GripperControl::Request  &req, arm_controller::GripperControl::Response &res)
{
    ...
    spinner.start();
    if(req.open == true){
        // auto move_group_gripper_goal = move_group_gripper->getCurrentJointValues();
        std::vector<double> move_group_gripper_goal = {9.470167157399345e-05, -9.470167157399345e-05, 9.470167157399345e-05, 
                                                        9.470167157399345e-05, -9.470167157399345e-05, 9.470167157399345e-05};
        move_group_gripper->setJointValueTarget(move_group_gripper_goal);
        move_group_gripper->move();
    }
    else{
        // auto move_group_gripper_goal = move_group_gripper->getCurrentJointValues();
        std::vector<double> move_group_gripper_goal = {0.39997525501279085, -0.39997525501279085, 0.39997525501279085, 
                                                        0.39997525501279085, -0.39997525501279085, 0.39997525501279085};
        move_group_gripper->setJointValueTarget(move_group_gripper_goal);
        move_group_gripper->move();
    }
    spinner.stop();
