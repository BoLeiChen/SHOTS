...
ros::Publisher pubLaserCloudCrop;
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloud2) {
  ...
  for (int i = 0; i < laserCloudSize; i++) {
    ...
    if (pointZ - vehicleZ > minRelZ - disRatioZ * dis &&
      ...
      sensor_msgs::PointCloud2 cropCloud2;
      pcl::toROSMsg(*laserCloudCrop, cropCloud2);
      cropCloud2.header.stamp = ros::Time().fromSec(laserCloudTime);
      cropCloud2.header.frame_id = "/map";
      pubLaserCloudCrop.publish(cropCloud2);
  newlaserCloud = true;
}
...
int main(int argc, char **argv) {
  ...
  pubLaserCloudCrop =
      nh.advertise<sensor_msgs::PointCloud2>("/crop", 2);
